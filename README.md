...
Shows the thread pool work queue contents and summary statistics.

```
!til.threadpoolqueue
!til.threadpoolqueue -detail
```

| Option | Description |
|--------|-------------|
| `-detail` | Show detailed information about each queued work item |

Displays CPU utilization, min/max threads, running/existing/goal thread counts, and enumerates queued work items. Items are identified as either Task or WorkItem, and grouped by callback method.

---

#### threadpoolstats (tps)

Shows running thread pool statistics.

```
!til.threadpoolstats
```

Displays CPU utilization, min/max threads, thread availability, and queue length breakdown by WorkItem vs Task types.
...
### Memory Analysis -- Memory Graph (BFS Spanning Tree)

These commands use a BFS spanning tree, which is a simpler and faster approximation than the dominator tree. Each node has exactly one parent in the spanning tree, allowing inclusive byte attribution along a single parent path.

Run `buildmemorygraph` first, then use the analysis commands.

#### buildmemorygraph

Builds a streaming memory graph from all managed objects on the heap. Results are cached in memory.

```
!til.buildmemorygraph
```

Indexes every valid managed object, records references between them, and roots the graph from GC roots. Uses a compact streaming format to minimize memory overhead.

After building, prints: node count, edge count, type count, total size, and stream size in bytes.

---

#### typebytes

Displays a reference tree for instances of a specific type, showing how much memory is reachable under each instance in the BFS spanning tree.

**Requires:** `buildmemorygraph`

```
!til.typebytes <TypeName>
!til.typebytes <TypeName> --max-type-roots 10
!til.typebytes <TypeName> --max-tree-nodes 500 --max-depth 20
!til.typebytes <TypeName> --quiet
```

| Option | Description |
|--------|-------------|
| `--max-type-roots N` | Maximum number of root instances to show, sorted by inclusive bytes descending. Default: 20. |
| `--max-tree-nodes N` | Maximum total tree nodes to print across all roots. Default: 2000. |
| `--max-depth N` | Maximum tree depth to expand. Default: 50. |
| `--quiet` | Suppress verbose output. |

Type matching: first attempts an exact (case-insensitive) match, then falls back to substring match.

For each root instance of the matching type, prints a subtree showing the inclusive bytes (object + all objects reachable under it in the spanning tree), exclusive bytes (the object itself), and type name at each node. Children are sorted by inclusive bytes descending.

---

#### totaltypebytes

Shows inclusive and exclusive byte totals for every type, aggregated across all instances.
...
Builds the same BFS spanning tree and computes inclusive bytes per node, then sums per type. Output columns are tab-separated: inclusive bytes, exclusive bytes, type name. Sorted by inclusive bytes descending.

---

### Inclusive Bytes (Standalone)

#### inclusivebytes

Calculates inclusive memory usage by type by walking object references from GC and static-field roots. This is a standalone command that does not require `buildmemorygraph` or `buildobjectgraph`.

```
!til.inclusivebytes
!til.inclusivebytes -n 50
```

| Option | Description |
|--------|-------------|
| `-n N` | Number of top types to show. Default: 20. |

Builds a graph from static field roots and heap roots, then walks references depth-first. For each type, computes:
- **Inclusive bytes**: the object plus all objects it references (directly or indirectly).
- **Exclusive bytes**: the object itself.

Shows the top N types sorted by inclusive bytes.
